...
def calc_PL_fixpar(
    ...
    # ------------------------------------------------------------------
    temp_max = np.minimum((1.0 + profile_width) * profile_params, upper)
    ...
    for par_i in range(num_profile):
        if par_i == 9:
            ...
            center = profile_params[par_i]

            left = np.linspace(PL_min[0], center, Nhalf + 1)
            right = np.linspace(center, PL_max[0], Nhalf + 1)

            par_space = np.concatenate([left[:-1], right])
...
with open('syntheticData\\set1_' + D + dpoints + '.pkl', 'rb') as f: #'/work/aayres/syntheticData/TRUE_SOL' + D + dpoints + '.pkl', 'rb') as f:
